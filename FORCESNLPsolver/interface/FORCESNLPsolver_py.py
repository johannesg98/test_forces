...
_lib = ctypes.CDLL(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../lib_target/libFORCESNLPsolver.so')))
csolver = getattr(_lib,'FORCESNLPsolver_solve')

_lib_interface = ctypes.CDLL(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../libInterface.so')))
...
a Python wrapper for a fast solver generated by FORCESPRO v6.2.0
 ...
 See also COPYING
    ...
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.FORCESNLPsolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp) , _lib_interface.FORCESNLPsolver_adtool2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.FORCESNLPsolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None , _lib_interface.FORCESNLPsolver_adtool2forces)
            else:
                exitflag = _lib.FORCESNLPsolver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp , _lib_interface.FORCESNLPsolver_adtool2forces)
            libc.fclose(fp)
